...
from funcoes import parse_csv_items
...
def carga_raw_items():
    ...
    with beam.Pipeline() as pipeline:
        ...
        csv_lines = (
            pipeline
            | 'Read CSV' >> beam.io.ReadFromText('gs://olist_brasil_project/tranzient/olist_order_items_dataset.csv', skip_header_lines=1)
        )
        
        # Process the CSV data (you can modify this step according to your needs)
        processed_data = (
            csv_lines
            | 'Parse CSV' >> beam.Map(parse_csv_items)
            | "Format Output items" >> beam.Map(lambda element: list(element.values()))
            | beam.ParDo(ToList())
        )
        ...
        output_path = 'gs://olist_brasil_project/raw/up_items.parquet'
        pq.write_table(table, output_path)


    with beam.Pipeline() as pipeline:
        # Leia o primeiro arquivo Parquet
        input_parquet_file = 'gs://olist_brasil_project/raw/items.parquet'
        file_exists = gcsio.GcsIO().exists(input_parquet_file)
        if file_exists:
            data1 = pipeline | 'Read Parquet 1' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/items.parquet')
            data2 = pipeline | 'Read Parquet 2' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/up_items.parquet')
        else:
            data1 = pipeline | 'Read Parquet 2' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/up_items.parquet')
            data2 = data1
        ...
        # Escrever a tabela em um arquivo Parquet
        output_path = 'gs://olist_brasil_project/raw/items.parquet'
        pq.write_table(table, output_path)



        # Defina o caminho completo para o arquivo Parquet no Cloud Storage
        parquet_file = 'gs://olist_brasil_project/raw/up_items.parquet'
        ...
        print(f'{registraDataLog()} - Processo de carga RAW items - OK')
