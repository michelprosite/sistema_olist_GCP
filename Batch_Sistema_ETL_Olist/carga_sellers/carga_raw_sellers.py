...
from funcoes import parse_csv_sellers
...
def carga_raw_sellers():
    # Define o pipeline
    with beam.Pipeline() as pipeline:
        # Read the CSV file
        csv_lines = (
            pipeline
            | 'Read CSV' >> beam.io.ReadFromText('gs://olist_brasil_project/tranzient/olist_sellers_dataset.csv', skip_header_lines=1)
        )
        
        # Process the CSV data (you can modify this step according to your needs)
        processed_data = (
            csv_lines
            | 'Parse CSV' >> beam.Map(parse_csv_sellers)
            | "Format Output sellers" >> beam.Map(lambda element: list(element.values()))
            | beam.ParDo(ToList())
        )
        ...
        output_path = 'gs://olist_brasil_project/raw/up_sellers.parquet'
        pq.write_table(table, output_path)


    with beam.Pipeline() as pipeline:
        # Leia o primeiro arquivo Parquet
        input_parquet_file = 'gs://olist_brasil_project/raw/sellers.parquet'
        file_exists = gcsio.GcsIO().exists(input_parquet_file)
        if file_exists:
            data1 = pipeline | 'Read Parquet 1' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/sellers.parquet')
            data2 = pipeline | 'Read Parquet 2' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/up_sellers.parquet')
        else:
            data1 = pipeline | 'Read Parquet 2' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/up_sellers.parquet')
            data2 = data1
        ...
        # Escrever a tabela em um arquivo Parquet
        output_path = 'gs://olist_brasil_project/raw/sellers.parquet'
        pq.write_table(table, output_path)



        # Defina o caminho completo para o arquivo Parquet no Cloud Storage
        parquet_file = 'gs://olist_brasil_project/raw/up_sellers.parquet'
        ...
        print(f'{registraDataLog()} - Processo de carga RAW sellers - OK')
