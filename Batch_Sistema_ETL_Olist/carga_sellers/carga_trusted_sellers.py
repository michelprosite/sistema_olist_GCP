...
def carga_trusted_sellers():

    with beam.Pipeline() as pipeline:
        # Leia o primeiro arquivo Parquet
        input_parquet_file = 'gs://olist_brasil_project/trusted/sellers.parquet'
        file_exists = gcsio.GcsIO().exists(input_parquet_file)
        if file_exists:
            data1 = pipeline | 'Read Parquet 1' >> parquetio.ReadFromParquet('gs://olist_brasil_project/trusted/sellers.parquet')
            data2 = pipeline | 'Read Parquet 2' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/sellers.parquet')
        else:
            data1 = pipeline | 'Read Parquet 2' >> parquetio.ReadFromParquet('gs://olist_brasil_project/raw/sellers.parquet')
            data2 = data1
        ...
        output_path = 'gs://olist_brasil_project/trusted/sellers.parquet'
        pq.write_table(table, output_path)


        print(f'{registraDataLog()} - Processo de carga TRUSTED sellers - OK')
